...
type ButtonType = {children?: ChildType, className?: string, variant?: ButtonVariant}

export default function Button({children, className = "", variant = "primary"}: ButtonType) {
    const buttonClasses = [];
    ...
    return (
        <button value="Log In" className={buttonClasses.join(" ")}>
            {children}
        </button>
    )
}